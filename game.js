...
const STAGE_CONFIG = [
    { id: "drums",  label: "Drums",              badge: "DRUMS",  hint: "Just the rhythm section." },
    ...
    { id: "other",  label: "+ Synths & Strings",  badge: "SYNTHS", hint: "Remaining melody instruments." },
    { id: "vocals", label: "+ Vocals",            badge: "VOX",    hint: "Vocals enter the mix." },
    { id: "full",   label: "Full Song",           badge: "FULL",   hint: "The complete, original recording." },
];
...
function getDailyInfo(songs) {
    ...
    return {
        number: dayIndex + 1,
        song:   songs[dayIndex % songs.length],
    };
}
...
async function loadData() {
    try {
        ...
        if (!state.songs.length) {
            throw new Error("dataComp.json has no songs. Run prepare_songs.py first.");
        }
        ...
        console.error(err);
        $("clue-content").innerHTML =
            `<div class="load-error">
                Could not load songs.<br><br>
                Run <code>python prepare_songs.py</code> then push
                <code>dataComp.json</code> and <code>audio/</code> to GitHub.
...
function playCurrentClue() {
    ...
    const stagePath = state.currentSong.stages[state.clueIndex];
    if (!stagePath) return;
    ...
    state.audio.addEventListener("ended", () => {
        state.isPlaying = false;
        renderTrackPanel();
        $("play-clue-btn").textContent =
            "Play Clue " + (state.clueIndex + 1) + " again";
    });

    state.audio.play().catch(err => {
        console.warn("Audio playback error:", err);
        setFeedback("Could not play audio. Check that the file exists on GitHub.", "error");
    });

    state.isPlaying = true;
    renderTrackPanel();
    $("play-clue-btn").textContent =
        "Playing Clue " + (state.clueIndex + 1) + " of " + TOTAL_CLUES + "...";
}
...
function renderTrackPanel() {
    ...
    for (let i = 0; i < STAGE_CONFIG.length; i++) {
        ...
        let rowState = "locked";
        if (i < state.clueIndex) {
            rowState = "done";
        } else if (i === state.clueIndex) {
            rowState = state.isPlaying ? "playing" : "active";
        }
        ...
        html += `
            <div class="track track-${rowState}" data-stem="${cfg.id}">
                ...
                <div class="track-body">
                    ...
                    ${rowState !== "locked"
                        ? `<div class="track-hint">${cfg.hint}</div>`
                        : ""}
                </div>
...
function checkGuess() {
    ...
    const correct =
        typed === answer ||
        (typed.length >= 4 && answer.includes(typed));

    if (correct) {
        handleCorrect();
    } else if (state.clueIndex < STAGE_CONFIG.length - 1) {
        ...
        renderPips();
        $("play-clue-btn").textContent =
            "Play Clue " + (state.clueIndex + 1) + " of " + TOTAL_CLUES;
    } else {
...
function buildResultGrid(won) {
    ...
    for (let i = 0; i < TOTAL_CLUES; i++) {
        if (i < state.clueIndex) {
            cells += `<span class="grid-cell cell-miss"></span>`;
        } else if (i === state.clueIndex && won) {
            cells += `<span class="grid-cell cell-win"></span>`;
        } else {
            cells += `<span class="grid-cell cell-empty"></span>`;
        }
    }
...
function buildShareText() {
    ...
    if (state.mode === "daily") {
        return [
            "StemGuesser Daily #" + state.dailyNumber,
            won
                ? "Solved on clue " + (state.clueIndex + 1) + " of " + TOTAL_CLUES
                : "Could not solve it today",
            grid,
            "Play: " + base,
        ].join("\n");
    }
    ...
    if (state.mode === "custom") {
        return [
            "Can you guess this song on StemGuesser?",
            base + "?mode=custom&song=" + state.currentSong.id,
        ].join("\n");
    }
...
function populateSongPicker() {
    ...
    if (!picker) return;
    picker.innerHTML = state.songs
        .map(s =>
            `<option value="${s.id}">${s.title} -- ${s.artists.join(", ")}</option>`
        )
        .join("");
    refreshCustomLink();
}
...
function setActiveModeButton() {
    ["btn-daily", "btn-random", "btn-custom"].forEach(id => {
        const el = $(id);
        if (el) el.classList.remove("mode-btn-active");
    });
    const activeId =
        state.mode === "daily"  ? "btn-daily"  :
        state.mode === "random" ? "btn-random" : "btn-custom";
    const el = $(activeId);
...
function switchToRandom() {
    state.mode = "random";
    setActiveModeButton();
    history.replaceState(null, "", window.location.pathname + "?mode=random");
    startRound();
}
